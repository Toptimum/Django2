...
action_1__price = When(action_1__condition, then=F('product__price_product') * F('quantity') * action_1__discount)

action_2__price = When(action_2__condition, then=F('product__price_product') * F('quantity') * -action_2__discount)

action_expired__price = When(action_expired__condition,
                             then=F('product__price_product') * F('quantity') * action_expired__discount)
...
for orderitem in test_orderss:
    print(f'{orderitem.action_order:2}: заказ №{orderitem.pk:3}:\
           {orderitem.product.name_product:15}: скидка\
